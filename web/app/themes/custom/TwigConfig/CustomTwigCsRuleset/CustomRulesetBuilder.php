...
namespace Atlantic\Themes\_NAMESPACE_\TwigConfig\CustomTwigCsRuleset;
...
class CustomRulesetBuilder
{
    ...
    public function binaryOpSpace(string $opName, string $spaceChar = '➀', string $spaceChar2 = '➁'): Handler
    {
        return $this
            ->handle()
            ->delegate('$', 'expr')
            ->enforceSpaceOrLineBreak($spaceChar, $this->config['binary']['before_op'], sprintf('There should be %%quantity%% space(s) between the "%s" operator and its left operand.', $opName))
            ->enforceSpaceOrLineBreak($spaceChar2, $this->config['binary']['after_op'], sprintf('There should be %%quantity%% space(s) between the "%s" operator and its right operand.', $opName))
        ;
    }
    ...
    public function build(): array
    {
        $tags = self::using(self::TAGS_VARS, [
            ...
            ['<➀set➊@➋=➌$➁>', $this
                ...
                ->enforceSpaceOrLineBreak('➌', $this->config['set']['after_equal'], 'There should be %quantity% space(s) after the "=".'),
            ],
            ['<➀@➁>', $this->noArgTag()],
        ...
        $ops = $this->using(self::OP_VARS, [
            ...
            ['$➀\|➁$', Handler::create()
                ->delegate('$', 'expr')
                ->enforceSpaceOrLineBreak('', $this->config['property']['before_|'], 'There should be %quantity% space(s) before the "|".')
                ->enforceSpaceOrLineBreak('', $this->config['property']['after_|'], 'There should be %quantity% space(s) after the "|".'),
            ],
            ['$➀\.➁$', Handler::create()
                ->delegate('$', 'expr')
